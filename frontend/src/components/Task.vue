<template>
    <div class="fixed z-50 top-0 left-0 right-0 bottom-0 m-auto w-2/3 h-[800px] bg-white border border-gray-400 flex">
        ...
        <!-- splide images -->
        <Splide :options="{ rewind: true }" aria-label="Vue Splide Example" class=" w-3/5 h-[800px] bg-yellow-500">
            <SplideSlide v-for="image in Object.assign({}, passData.task.images)" :key="image.id" class=" w-full h-full">
                <img :src="require(`./../assets/${image.name}`)" alt="Sample 1" class=" w-full h-[800px]">
            </SplideSlide>
        </Splide>
