<template>
    <div class=" bg-white border border-gray-400 w-1/4 h-96 p-2">
        <p class=" text-sm">Suggest friends for you</p>
        <div v-if="fetchUserData.users.length === 0" class="mt-2">
        ...
        <ul v-else class=" mt-3">
            <li v-for="user in fetchUserData.users" :key="user" class=" text-semibold">
                {{ user.username }}
            </li>
        </ul>
    </div>
...
// onMounted( async() => {
    ...
    // emit('sendToDashboard', fetchUser.data)
// })
</script>